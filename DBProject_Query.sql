...
SELECT MONTH(hst.SalesTransactionDate) AS [MONTH],
	MIN(dst.ItemQuantity) AS [Minimum Quantity Sold],
	MAX(dst.ItemQuantity) AS [Maximum Quantity Sold]
...
WHERE YEAR(hst.SalesTransactionDate) = 2019 AND
    mi.ItemTypeName NOT IN('Food', 'Drink')
GROUP BY hst.SalesTransactionDate
...
SELECT UPPER(CONCAT(SUBSTRING(c.CustomerName, 1, 1), SUBSTRING(c.CustomerName, LEN(c.CustomerName), 1))) AS [Customer Initial],
    CONVERT(VARCHAR, hst.SalesTransactionDate, 107) AS [Transaction Date],
    dst.ItemQuantity
